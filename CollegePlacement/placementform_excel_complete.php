...
$qryreport = mysql_query("select * from placementform where department_id  = '$get_id'") or die(mysql_error());
...
$pgsem6 =$row['pgsem6'];
   ...
   $worksheet1->write_string($j,2,"$address");
   $worksheet1->write_string($j,3,"$contact");
      $worksheet1->write_string($j,4,"$email");
      $worksheet1->write_string($j,5,"$sex");

   $worksheet1->write_string($j,6,"$department");
   $worksheet1->write_string($j,7,"$sy");
   $worksheet1->write_string($j,8,"$sslc");
   $worksheet1->write_string($j,9,"$puc");
	   $worksheet1->write_string($j,10,"$sem1");
	   $worksheet1->write_string($j,11,"$sem2");
	   $worksheet1->write_string($j,12,"$sem3");
	   $worksheet1->write_string($j,13,"$sem4");
	   $worksheet1->write_string($j,14,"$sem5");
	   $worksheet1->write_string($j,15,"$sem6");
	   	   $worksheet1->write_string($j,16,"$degree");

	   $worksheet1->write_string($j,17,"$pgsem1");
	   $worksheet1->write_string($j,18,"$pgsem2");
	   $worksheet1->write_string($j,19,"$pgsem3");
	   $worksheet1->write_string($j,20,"$pgsem4");
	   $worksheet1->write_string($j,21,"$pgsem5");
	   $worksheet1->write_string($j,22,"$pgsem6");
	   $worksheet1->write_string($j,23,"$pg");
