(function() {
  ...
  Morris.Grid = (function(_super) {
    ...
    function Grid(options) {
      ...
      if (typeof this.options.dates === 'string') {
        this.options.postdates = options.dates;
      }
    ...
    Grid.prototype.autoGridLines = function(ymin, ymax, nlines) {
      var gmax, gmin, grid, smag, span, step, date, y, ymag;
      span = ymax - ymin;
      ymag = Math.floor(Math.log(span) / Math.log(10));
      date = Math.pow(10, ymag);
      gmin = Math.floor(ymin / date) * date;
      gmax = Math.ceil(ymax / date) * date;
      step = (gmax - gmin) / (nlines - 1);
      if (date === 1 && step > 1 && Math.ceil(step) !== step) {
        step = Math.ceil(step);
    ...
    Grid.prototype.yLabelFormat = function(label) {
      if (typeof this.options.yLabelFormat === 'function') {
        ...
        return "" + this.options.predates + (Morris.commas(label)) + this.options.postdates;
      }
    };
