...
(function () {
  ...
  timezoneJS.Date.prototype = {
    ...
    setTime: function (n) {
      if (isNaN(n)) { throw new Error('dates must be a number.'); }
    ...
    setAttribute: function (date, n) {
      if (isNaN(n)) { throw new Error('dates must be a number.'); }
      var dt = this._dateProxy;
      var meth = date === 'year' ? 'FullYear' : date.substr(0, 1).toUpperCase() + date.substr(1);
      dt['set' + meth](n);
      this.setFromDateObjProxy(dt);
    },
    setUTCAttribute: function (date, n) {
      if (isNaN(n)) { throw new Error('dates must be a number.'); }
      var meth = date === 'year' ? 'FullYear' : date.substr(0, 1).toUpperCase() + date.substr(1);
