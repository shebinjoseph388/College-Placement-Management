			<form id="signin_teacher" class="form-signin" method="post">
					<h3 class="form-signin-heading"><i class="icon-lock"></i> New Company Register</h3>
					<input type="text" class="input-block-level"  name="firstname" placeholder="Firstname" maxlength="40" required>
					<input type="text" class="input-block-level"  name="lastname" maxlength="40" placeholder="Lastname" required>
			...
			jQuery("#signin_teacher").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Welcome to Centre for Placements and Corporate Relations(CPCR)", { header: 'Sign up Success' });
						var delay = 1000;
