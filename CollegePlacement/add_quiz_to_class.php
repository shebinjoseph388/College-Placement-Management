					...
					<?php } ?>
										...
										if (isset($_POST['save'])){
											...
											$id=$_POST['selector'];
													...
													$name_notification  = 'Add Exam';
													
											$N = count($id);
											for($i=0; $i < $N; $i++)
											{
												mysql_query("insert into class_quiz (teacher_class_id,quiz_time,quiz_id) values('$id[$i]','$time','$quiz_id')")or die(mysql_error());
												mysql_query("insert into notification (teacher_class_id,notification,date_of_notification,link) value('$id[$i]','$name_notification',NOW(),'Exam-View-List')")or die(mysql_error());
											} ?>
											<script>
