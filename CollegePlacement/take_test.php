...
<input type="hidden" id="quiz_id" value="<?php echo $class_quiz_id; ?>">
...
?>
										...
										setInterval(function(){
											...
											jQuery("#timer").load("timer.ajax.php?quiz_id="+quiz_id+"");	
											if(timer == 0){
												jQuery(".questions-table input").hide();
...
<form action="Take-Exam<?php echo '?id='.$get_id; ?>&<?php echo 'class_quiz_id='.$class_quiz_id; ?>&<?php echo 'test=done' ?>&<?php echo 'quiz_id='.$quiz_id; ?>&<?php echo 'quiz_time='.$quiz_time; ?>" name="testform" method="POST" id="test-form">
<?php
...
?>
										<h3>Test: <b><?php echo $rowa['quiz_title'];?></b></h3>
										<p></p>
...
<?php
	...
	<a href="Exam-View-List<?php echo '?id='.$get_id; ?>"><i class="icon-arrow-left"></i> Back</a>
	<center>
...
?>
<script>
	  window.location = 'Exam-View-List<?php echo '?id='.$get_id; ?>'; 
</script>
<?php
	} /* else { */
?>
